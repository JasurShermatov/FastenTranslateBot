        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    url, headers=self.headers, ssl=ssl_context
                ) as response:
                    if response.status != 200:
                        logger.error(
                            f"Cambridge Dictionary API xatosi: {response.status} - {url}"
                        )
                        return {
                            "success": False,
            ...
            return {"success": False, "error": f"So'rovda xatolik yuz berdi: {str(e)}"}
        ...
        try:
            ...
            if uk_audio and uk_audio.get("src"):
                result["pronunciations"]["uk"] = (
                    "https://dictionary.cambridge.org" + uk_audio.get("src")
                )
            ...
            if us_audio and us_audio.get("src"):
                result["pronunciations"]["us"] = (
                    "https://dictionary.cambridge.org" + us_audio.get("src")
                )
